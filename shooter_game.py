...
win_message = font1.render("YOU WIN BRO!", True, (255,255,255))
...
rel_time = False #flag is reloaded
num_fire = 0     #number of shots
...
while run == True:
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_SPACE:
                #Check how many shots fired and if it is already reloading
                if num_fire < 5 and rel_time == False:
                    num_fire += 1
                    fire_sound.play()
                    rocket.fire()

                #If shot 5 times, then reloading (cooldown)
                if num_fire >-5 and rel_time == False:
                    last_timer = timer()
                    rel_time = True
